// app.js v5 — creación abierta de users + indicador de projectId
...
btnInitBoard?.addEventListener('click', async ()=>{
  try{
    ...
    alert('Tablero creado. ¡Listo!');
  }catch(e){ console.error(e); alert('No se pudo crear el tablero: '+e.message); }
});
...
function setupSubs(){
  ...
  unsubBoard = onSnapshot(bref, snap=>{
    if(!snap.exists()){ estadoLabel.textContent='Sin inicializar (crear doc tableros/llenadora)'; initBox.style.display='flex'; return; }
  ...
  unsubLogs = onSnapshot(q, snap=>{
    logList.innerHTML='';
    if(snap.empty){ const empty=document.createElement('div'); empty.style.opacity=.7; empty.textContent='Sin movimientos aún.'; logList.appendChild(empty); return; }
    snap.forEach(docSnap=>{
      ...
      row.innerHTML=`<time>${ts.toLocaleString()}</time><div><strong>${prettyRole(item.role)}</strong> → <em>${item.action}</em> · Estado: <strong>${labelFromKey(item.to)}</strong>${nota}</div>`;
...
function render(){
  if(USER && ROLE){ loginBox.style.display='none'; loggedBox.style.display='block'; roleBadge.hidden=false; roleName.textContent=prettyRole(ROLE); roleHint.textContent=USER.isAnonymous?'Anon demo':'activo'; }
  else { loginBox.style.display='flex'; loggedBox.style.display='none'; roleBadge.hidden=true; }
  renderStepper(); renderActions(); estadoLabel.textContent=labelFromKey(currentState);
}
function renderStepper(){ stepperBox.innerHTML=''; const curIdx=stateIndex(currentState); STATES.forEach((s,idx)=>{ const el=document.createElement('div'); el.className='step'; if(idx<curIdx) el.classList.add('done'); if(idx===curIdx) el.classList.add('active'); el.innerHTML=`<div class="dot"></div><div style="font-size:12px">${idx+1}. ${s.label}</div>`; stepperBox.appendChild(el); }); }
function renderActions(){ actionsBox.innerHTML=''; if(!USER||!ROLE){ return; } const opts=(TRANSITIONS[ROLE]&&TRANSITIONS[ROLE][currentState])||[]; if(!opts.length){ const none=document.createElement('div'); none.style.opacity=.75; none.textContent='Sin acciones disponibles para tu sector en este estado.'; actionsBox.appendChild(none); } else { for(const op of opts){ const b=document.createElement('button'); b.className='btn primary row'; b.textContent=op.action; b.addEventListener('click',()=>applyTransition(op.to, op.action)); actionsBox.appendChild(b); } } }
...
async function applyTransition(nextKey, actionLabel){
  ...
  try{
    await runTransaction(db, async (tx)=>{
      ...
      await addDoc(logsCol, { ts: serverTimestamp(), uid:(auth.currentUser?.uid)||'anon', role: ROLE, from: cur, to: nextKey, action: actionLabel, note });
    });
